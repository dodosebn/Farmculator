...
const Interacto = () => {
  ...
  const todos = [
    { name: "Records", path: '/in/Dashboard', icon: <IoDocumentLockSharp size={20} />},
    { name: "Farm Advisor", path: '/in/farm-advisor', icon: <FaTrainTram size={20}/> },
    { name: "Logout", icon: <CiLogout />, action: openLogoutConfirm }
  ];
  return (
    <div className="flex flex-col md:items-center items-start px-6 md:px-0 gap-3 relative">
      <div className="absolute md:top-7 top-14 right-7">
        <button 
          ...
          title="Logout"
        >
          <span className="hidden md:flex items-center gap-2 px-3 py-2 text-gray-700
           hover:text-black transition-colors">
            <CiLogout size={18} />
            Logout
          </span>
          <CiLogout size={24} className="md:hidden text-gray-600 hover:text-black
           transition-colors" />
          <span className="
      ...
      <p className="text-lg text-center flex mx-auto font-semibold mt-2">What do you want to do today?</p>
      
      <div className="md:flex gap-2 hidden">
        {Todos.filter(todo => todo.name !== "Logout").map((todo, index) => (
          todo.action ? ( 
            <button key={index} onClick={todo.action} className="cursor-pointer">
